...
%Nota: 7680 muestras por subframe
%% Configuration
...
slMode              = 4;                        % Sidelink Mode: 1, 2, 3 or 4. For SL-DCH there is no difference. For SL-BCH 1 is equivalent with 2 and 3 is equivalent with 4. Set 1 or 2 for D2D, 3 or 4 for V2V (default : 1)
syncOffsetIndicator = 0;                        % SL-SyncConfig: Offset indicator for sync subframe with respect to subframe #0: 0..39 (default : 0)
%Nota: valor en subframes del primer subframe donde empezar a buscar
...
%Nota: El periodo de sincronizacion no esta claro en la literatura escogida
%entre 40 y 160 ms para V2X.

% SL DISCOVERY Configuration 
discPeriod_r12      = 32;                       % SL-DiscResourcePool: 32,64,128,256,512,1024 radio frames
offsetIndicator_r12 = 0;                        % SL-OffsetIndicator : 0..10239 (time offset) 
%Nota: valor en subframes
subframeBitmap_r12  = repmat([0;1;1;1;0],8,1);  % SL-TF-ResourceConfig: size 40 (subframe resources) %Nota: pattern to repeat
numRepetition_r12   = 5;                        % SL-DiscResourcePool: 1..5 (subframeBitmap_r12 repetitions)
%Para elegir los prbs en funcion de subcanales se pueden usar las formulas de 36.331
prb_Start_r12       = 2;                        % SL-TF-ResourceConfig: 0.99
prb_End_r12         = 22;                       % SL-TF-ResourceConfig: 0.99
prb_Num_r12         = 5;                       % SL-TF-ResourceConfig: 1..100             
numRetx_r12         = 2;                        % SL-DiscResourcePool: 0..3 (msg retransmissions) %%Nota:¿ maximo para v2x= 1? para comunicaciones seguro, discovery pendiente de comprobar

classdef SL_V2XCommunication
    ...
    methods
        ...
        function h = GetV2XCommResourcePool (h)
            ...
            %Nota: habria que eliminar tambien los downlink and special
            %subframes si estuvieramos en TDD
            %Nota: Calculo de subframes reservados:
            Nreserved = mod(10240-Nslss,length(h.sl_Subframe_r14)); % Nota: length(h.sl_Subframe_r14) es el tamaño del pattern de sidelink subframes
            ...
            if h.adjacencyPSCCH_PSSCH_r14 % keep first two PRBs of already defined PSSCH pool
                ...
                %Nota: la posicion de los SCI es al final en numeracion
            end
...
%             keyboard
        ...
        function DecodeData (h, subframe_counter, prbs_data, nXID, rxConfig, rx_input_grid)
            ...
            if (CRCerror_flg==0 && ~all(current_datatb_rec==0))
                ...
                %Nota: decodificando el mensaje del todo REVISAR
                fprintf(' SL-SCH TB random message %x (hex format) received\n', bitTodec(current_datatb_rec(1:50),true));
            else
        ...
        function [evm, biterrs, bitsim] = DecodeQualEstimate (h, decoded_bit_seq, recovered_qpskin_seq, nXID, nssfPSSCH, mode)
            %Nota: compara los mensajes codificados
